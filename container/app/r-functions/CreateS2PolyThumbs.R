...
optList <- list(
  ...
  optparse::make_option("--polyID",type="character"), #ID of selected polygon or polygons (separated by ",")
...
#Assign variables
segmentation_polygon<-st_read(cmdargs$sf) #May need to change if using different format of spatial framework
...
#if a list of polygons supplied then split the list
if(grepl(polyID, pattern=",")){
  polyID<-stringr::str_split(polyID, pattern=",")%>% unlist() %>% trimws()
} else {
  polyID<-polyID
}

  #Create list of the polygons to iterate through
  polygons<-as.list(polyID)

  # Iterate through polygons IDs
  polythumbs<-purrr::map(polygons, .f=function(polyID){
    ...
    image_name <- paste0(outDir, "/", imgName, ".png")
