##Assign frame ID (equivalent to OSGB 100km grid)
...
optList <- list(
  ...
  optparse::make_option("--s1map", type= "character"), # path to the folder containing S1 OSGB map 
...
FramePolygonIDs<-function(sf, idfield, habfield, habremove, s1map, outpath){
  ...
  st_write(poly_clean, file.path(paste0(outpath, sf_name, ".shp")))
  
  ##Match polygons to Sentinel frame 
  
  #Get OSGB 100km grid map 
  ...
  s1_frames <- s1_frames %>% sf::st_transform(s1_frames,crs=st_crs(poly_clean))
  
  #Iterate through granules
  
  for (i in 1:nrow(s1_frames)){
    
    overlap<-st_join(s1_frames[i,],poly_clean, join=st_contains, left=TRUE)
    ...
    #Extract granule ID
    frameID<-as.character(unique(overlap$gridref))
    
    #Put results together as array
    out<-data.frame(FrameID = frameID,
                    POLYID = c(polys))
